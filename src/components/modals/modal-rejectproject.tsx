...
import { useActionProject } from "@/_hooks/useProjectStatus";
...
const ModalRejectProject = ({ isOpen, onClose }: ModalProp) => {
  const params = useParams();
  const id = (params?.slug as string[])?.[1];
  const [textArea, setTextArea] = useState("");
  const { rejectMutation } = useActionProject();
  const { closeModal } = useModalStore();

  const handleClick = () => {
    rejectMutation.mutate({ data: textArea, id });
  };
...
export default ModalRejectProject;
